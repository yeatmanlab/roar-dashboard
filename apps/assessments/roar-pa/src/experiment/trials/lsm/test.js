...
export const lsmTestTrials = {
  timeline: [
    ...getTestTrials(),
    {
      ...
      on_finish: (data) => {
        ...
        if (!store.session('config').isAdaptive) {
          ...
          store.session.set('currentStimulus', corpus.test_LSM[store.session('currentCorpusIndex')]);
        }
    ...
    },
    {
      type: jsPsychAudioButtonResponse,
      stimulus: () => {
        const reward_sound = ['reward4_60', 'reward5_60', 'reward6_60'];
      ...
      prompt: () => {
        ...
        if (store.session.get('config').story) {
          rewardPic = ['carrotsPlus3', 'carrotsPlus2', 'carrotsPlus1'];
        } else {
    ...
    {
      timeline: [lsmBreak],
      conditional_function: () => breakTrialConditionalFunction('lsm'),
    },
  ],
  loop_function: () => testLoopFunction('lsm'),
};
