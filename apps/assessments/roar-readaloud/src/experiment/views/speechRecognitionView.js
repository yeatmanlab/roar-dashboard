...
import speechRecognition_page from './speechRecognition.html';

window.store = store;

export async function speechRecognitionView() {
  // Load the existing HTML page
  // const response = await fetch("https://eyetrackingdata.blob.core.windows.net/public/views/menu.html");
  const speechRecognitionViewHtml = speechRecognition_page;

  // Create a div and set its innerHTML to the loaded HTML content
  const speechRecognitionPage = document.createElement('div');
  speechRecognitionPage.innerHTML = speechRecognitionViewHtml;

  // Append the confirmation page to the body or a specific element
  document.body.appendChild(speechRecognitionPage);

  // Load and execute external scripts
  await loadExternalScripts(speechRecognitionPage);
  executeInlineScripts(speechRecognitionPage);
