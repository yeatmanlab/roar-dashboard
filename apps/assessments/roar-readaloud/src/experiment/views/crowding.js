  ...
  if (eyeMoved & (count_eyeMoved < 15)) {
    ...
    if (type === "Test") {
      document.getElementById(
        "answerTitle",
      ).textContent = `Select Answer (Q${count_initTrials}/${
...
export function headinPlace() {
  return (
    isAreaWithinRange(fixedContour, movingContour) &&
    isCentroidInsidePath(fixedContour, movingContour)
  );
}
...
export function shuffleStimulus() {
  if (testConfig.MCL_config && testConfig.MCL_repeat) {
    for (let i = 0; i < testConfig.MCL_repeat; i++) {
      ...
      if (type === "Practice") {
        shuffled = testConfig.MCL_config_practice.sort(
          () => Math.random() - 0.5,
        );
...
export function stimulusTemplate(
  ...
  createStimulus(
    "leftStim",
    left,
    container.offsetWidth / 2 - offsetX,
    container.offsetHeight / 2,
  );
  createStimulus(
    "rightStim",
