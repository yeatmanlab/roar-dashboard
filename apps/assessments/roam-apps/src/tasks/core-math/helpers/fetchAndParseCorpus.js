...
import { camelize, getGrade } from "@bdelab/roar-utils";
...
const transformCSV = (csvInput) => {
  ...
  for (var i = 0; i < csvInput.length; i++) {
    const newRow = {
      ...
      response_format: csvInput[i].response_format
        ? csvInput[i].response_format.split(";")
        : "",
...
const transformInstructions = (csvInput) => {
  ...
  for (var i = 0; i < csvInput.length; i++) {
    const newRow = {
      ...
      item_raw: csvInput[i].item,
      target: csvInput[i].target
        ? csvInput[i].target.toString().split(";")
        : "",
      response_format: csvInput[i].response_format
        ? csvInput[i].response_format.split(";")
        : "",
...
export const fetchAndParseCorpusCoreMath = async (task, assets) => {
  ...
  if (store.session.get("config").userMode === "adaptive") {
    corpusLocation = {
      stimulus:
        ...
        lng +
        `/shared/corpora/core-math/items_cat.csv`,
      instructions:
        `https://storage.googleapis.com/roam-apps/` +
        lng +
        `/shared/corpora/core-math/items-instructions.csv`,
    };
  } else {
    corpusLocation = {
      stimulus:
  ...
  for (let i = 0; i < increment_list.length; i += 1) {
    ...
    if (
      ...
      if (current_item["item_type"] === "fractionInstruction") {
        ...
        if (current_item["lead_raw"].length > 0) {
          current_item["demoNumbers"] = current_item["lead_raw"][0].replace(
            /[^0-9]/g,
            "",
          );
        }
    ...
    current_item["countDownAppears"] =
      current_item["time_limit"] - current_item["countdown_time"] * 1000;
    ...
    if (
      subSkillRange[current_item["skill_category_camel"]].minGrade >
      current_item["b_grade"]
    ) {
      subSkillRange[current_item["skill_category_camel"]].minGrade =
        current_item["b_grade"];
    }
    if (
      subSkillRange[current_item["skill_category_camel"]].maxGrade <
      current_item["b_grade"]
    ) {
      subSkillRange[current_item["skill_category_camel"]].maxGrade =
        current_item["b_grade"];
  ...
  if (grade >= 5) {
    ...
    for (let i = 0; i < breakScreens.length; i++) {
      assets.default.shared.push(
        "core-math-break-screen-" + breakScreens[i] + ".png",
      );
      assets.default.languageSpecific.shared.push(
        "core-math-break-" + i + ".mp3",
      );
    }
    ...
    assets.default.languageSpecific.device.push("core-math-response-k4.gif");
    assets.default.languageSpecific.shared.push(
      "core-math-introduction-k2.mp3",
    );
    assets.default.languageSpecific.shared.push(
      "core-math-pretask-check-k2.mp3",
    );
