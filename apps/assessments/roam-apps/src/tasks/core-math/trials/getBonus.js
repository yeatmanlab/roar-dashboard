...
const ifMultiChoiceImage = createConditionalTrial('multiChoiceImage', multiChoiceImage);
...
const ifTextboxResponse = createConditionalTrial('textboxResponse', textboxResponse);
const ifKeyboardInstruction = createConditionalTrial('keyboardInstruction', keyboardInstruction);
const ifFractionInstruction = createConditionalTrial('fractionInstruction', fractionInstruction);

export const bonusLoop = (corpusName) => ({
  timeline: [
    updateStimulus(corpusName),
    ifKeyboardInstruction(corpusName, 'practice'),
    ifFractionInstruction(corpusName, 'practice'),
    ifMultiChoice(corpusName, 'test'),
    ifMultiChoiceImage(corpusName, 'test'),
    ifSelectDuck(corpusName, 'test'),
    ifGiveN(corpusName, 'test'),
    ifNumberLine(corpusName, 'test'),
    ifTextboxResponse(corpusName, 'test'),
    ifTimeoutFlash,
    ifBreakScreen,
  ],
  on_timeline_finish: () => {
  ...
  return {
    ...
    func: () => {
      ...
      for (let i = 0; i < corpus.length; i++) {
        if (corpus[i].skill_category_camel === bonusSkills[store.session.get('bonusIdx')]) {
          subCorpus.push(corpus[i]);
          if (firstFraction === -1 && corpus[i].response_format === 'fraction') {
        ...
        if (!store.session.get('keyboardInstructionDone') && corpus[i].item_type === 'keyboardInstruction') {
          keyboardInstructionIdx = i;
        }
        if (!store.session.get('fractionInstructionDone') && corpus[i].item_type === 'fractionInstruction') {
...
export const ifBonusBlock = (corpusName) => {
  return {
    ...
    conditional_function: () => {
      ...
      if (supportCategory === 'Needs Extra Support' || supportCategory === 'Developing Skill') {
        for (let key in gradeEstimateObject) {
          if (key !== 'composite') {
            let perCorrect =
              gradeEstimateObject[key].totalAttempted != 0
                ? gradeEstimateObject[key].totalCorrect / gradeEstimateObject[key].totalAttempted
                : 0;
