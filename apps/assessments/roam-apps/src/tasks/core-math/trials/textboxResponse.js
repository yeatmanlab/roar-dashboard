  ...
  let stim = {
    ...
    html: () => {
      ...
      if (currentItem.addImage) {
        questionHTML +=
          `<img src="${
            mediaAssets.images['coreMathProblem' + currentItem.problemID + 'Version' + currentItem.version]
          }" style="height:` +
      ...
      if (currentItem.response_format[0] === 'single') {
        ...
        for (let i = 0; i < currentItem.response_format.length; i++) {
          if (i == 1 && currentItem.response_format[0] == 'leadIn' && currentItem.response_format[1] == 'leadOut') {
            responseHTML += `<p>,</p>`;
          }
        ...
        responseHTML = `<div class="response-horizontal">` + responseHTML + `</div>`;
      }

      //add submit button if there's more than 1 textbox
      if (currentItem.response_format.length > 1 || currentItem.response_format[0] === 'fraction') {
    ...
    on_load: () => {
      ...
      if (responseFormat.length > 1) {
        document.getElementById('question_input_key_0').addEventListener('focus', function () {
          currentTextbox = 'textbox1';
        });
        document.getElementById('question_input_key_1').addEventListener('focus', function () {
      ...
      const storeKeyPress = (event) => {
        if (responseFormat[0] === 'fraction' && event.code === 'ArrowUp') {
        ...
        } else if (responseFormat[0] === 'fraction' && event.code === 'ArrowDown') {
          //to switch between response boxes in fraction problems
          storeKeyRT(event.key);
          event.preventDefault();
          ...
          element.focus();
          element.setSelectionRange(element.value.length, element.value.length);
        } else if (responseFormat.length == 2 && currentTextbox === 'textbox1' && event.code === 'ArrowRight') {
          ...
          element.focus();
          element.setSelectionRange(element.value.length, element.value.length);
        } else if (responseFormat.length == 2 && currentTextbox === 'textbox2' && event.code === 'ArrowLeft') {
          ...
          storeKeyRT(event.key);
          let textEmpty0 = document.getElementById('question_input_key_0').value.trim() === '';
          let textEmpty1 = document.getElementById('question_input_key_1').value.trim() === '';
          ...
          !(event.code === store.session.get('decimalCode') && event.key === store.session.get('decimalKey'))
        ) {
    ...
    on_finish: (data) => {
      ...
      const hasDecimalResponse = textboxVal.some((val) => typeof val === 'string' && val.includes(decimalKey));
      ...
      if (textboxVal.length != 0) {
        ...
        if (stimulus.response_format[0] === 'fraction') {
          ...
          if (hasDecimalResponse) {
            ...
            !stimulus.item_raw.includes(i18next.t('terms.simplifiedFraction')) &&
            correctCount === 0
          ) {
      ...
      let correct = correctCount === stimulus.target.length && !store.session.get('timeOut') ? 1 : 0;
    ...
    stimulus: () => {
      ...
      if (currentItem.response_format[0] === 'single') {
        ...
        responseHTML =
          ...
          <p class="warning-text" id="warning">${i18next.t('warning-mobile')}</p>`;
      } else {
        //For items which require a text value before the textbox such as x=
        for (let i = 0; i < currentItem.response_format.length; i++) {
          if (i == 1 && currentItem.response_format[0] == 'leadIn' && currentItem.response_format[1] == 'leadOut') {
        ...
        responseHTML =
          `<div class="response-horizontal">` +
          responseHTML +
          `</div><p class="warning-text" id="warning">${i18next.t('warning-mobile')}</p>`;
      }
    ...
    on_load: () => {
      ...
      function onKeyPress(button) {
        ...
        if (button === '{bksp}') {
          ...
          if (!store.session.get('warningVisible') && (responseFormat.length > 1 || responseFormat[0] === 'fraction')) {
            let textEmpty0 = document.getElementById('question_input_key_0').textContent.trim() === '';
            let textEmpty1 = document.getElementById('question_input_key_1').textContent.trim() === '';
            if (textEmpty0 != textEmpty1) {
              document.getElementById('warning').style.visibility = 'visible';
              store.session.set('warningVisible', true);
            } else {
