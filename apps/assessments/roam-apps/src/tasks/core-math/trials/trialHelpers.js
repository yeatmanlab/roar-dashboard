    ...
    func: () => {
      ...
      if (currentCorpus.length != 0) {
        if (currentCorpusName === "lower") {
          ...
          if (
            store.session.get("gradeEstimateObject").composite.totalCorrect !==
            store.session.get("itemsToCheck")
          ) {
      ...
      if (
        store.session.get("corpusComplete") &&
        currentCorpusName === store.session.get("corpusToRun")
      ) {
...
const checkEndBlock = (arr, stopCriterion, maxLength) => {
  ...
  if (
    (arr.length == maxLength &&
      arr.reduce((sum, current) => sum + current, 0) <= stopCriterion) ||
    (store.session.get("currentCorpus").length === 0 &&
      store.session.get("currentCorpusName") === "remaining")
  ) {
...
export const endGame = (arr, stopCriterion, maxLength) => {
  ...
  if (
    store.session.get("currentCorpusName") === "lower" &&
    store.session.get("currentCorpus").length === 0
  ) {
  ...
  if (
    store.session.get("endBlock") &&
    store.session.get("bonusSkills").length === store.session.get("bonusIdx")
  ) {
...
export const updateSkillScores = (correct, stimulus) => {
  ...
  if (stimulus.b_grade < grade) {
    ...
    for (let i = 0; i < stimulus["skill"].length; i++) {
      ...
      if (correct === 0) {
        scores[skill].totalIncorrectBelow =
          scores[skill].totalIncorrectBelow + 1;
      }

      scores[skill].totalAttemptedBelow = scores[skill].totalAttemptedBelow + 1;
      if (
        (scores[skill].totalAttemptedBelow <= 3 &&
          scores[skill].totalIncorrectBelow > 0) ||
        (scores[skill].totalAttemptedBelow > 3 &&
          scores[skill].totalIncorrectBelow > 1)
      ) {
