...
export const multiChoice = (corpusName, assessment_stage_val) => {
  let stim = {
    ...
    on_load: () => {
      ...
      if (
        ...
        if (store.session.get('nextStimulus').itemID === 327) {
          document.getElementById('jspsych-audio-multi-response-btngroup').classList.add(`grid-layout-long-compact`);
        } else {
          document.getElementById('jspsych-audio-multi-response-btngroup').classList.add(`grid-layout-long`);
        }
      } else if (
        store.session.get('nextStimulus').choices[0].includes('<k>') ||
        ...
        document.getElementById('jspsych-audio-multi-response-btngroup').classList.add(`grid-layout`);
      } else {
    ...
    on_finish: (data) => {
      ...
      let correct = data.button_response === stimulus.correctResponseNum && !store.session.get('timeOut') ? 1 : 0;
