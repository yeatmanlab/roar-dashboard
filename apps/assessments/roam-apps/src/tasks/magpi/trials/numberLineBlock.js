...
const numberLineBlock = (corpusName, assessment_stage_val) => {
  return {
    timeline: [updateStimulus(corpusName), numberLineSlider(assessment_stage_val), ifTimeoutFlash],
    loop_function: () => {
...
const initBlock = (corpusName) => {
  return {
    ...
    func: () => {
      ...
      let currentCorpus = store.session.get('corpusAll')[corpusName]['stimulus'][block];
...
export const numberLineOuterLoop = (corpusName, assessment_stage_val) => {
  return {
    timeline: [initBlock(corpusName), ifPractice(), ifDemo(), numberLineBlock(corpusName, assessment_stage_val)],
    loop_function: () => {
      store.session.transact('arrayIdx', (oldVal) => oldVal + 1);
      if (store.session.get('arrayIdx') === store.session.get('blockOrderNumLine')['stimulus'].length) {
        return false;
      }
      return true;
    },
