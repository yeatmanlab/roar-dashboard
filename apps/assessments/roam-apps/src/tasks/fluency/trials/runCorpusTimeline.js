...
export const numberMainOuterLoop = (corpusName, assessment_stage_val, responseMode) => {
  return {
    timeline: [initBlock(corpusName, responseMode), numberMainLoop(corpusName, assessment_stage_val, responseMode)],
    loop_function: () => {
      store.session.transact('arrayIdx', (oldVal) => oldVal + 1);

      if (store.session.get('arrayIdx') < store.session.get('timerDuration').length) {
...
const numberMainLoop = (corpusName, assessment_stage_val, responseMode) => {
  let timelineObj = [updateStimulus(corpusName), numberMainTimer(corpusName, assessment_stage_val), ifTimeoutFlash];
  if (responseMode.includes('afc')) {
    timelineObj = [updateStimulus(corpusName), numberMainNAFC(corpusName, assessment_stage_val), ifTimeoutFlash];
  }
  return {
