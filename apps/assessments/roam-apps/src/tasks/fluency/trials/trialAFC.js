        ...
        if (save_trial) {
          if (!store.session.get('responseModality')) {
            ...
            if (subCorpusName === 'rtControl_2afc') {
              store.session.transact('trialNumTotalControl2afc', (oldVal) => oldVal + 1);
            } else if (subCorpusName === 'rtControl_6afc') {
              store.session.transact('trialNumTotalControl6afc', (oldVal) => oldVal + 1);
            } else {
              store.session.transact('trialNumTotalAFC', (oldVal) => oldVal + 1);
              if (correct) {
                store.session.transact('totalCorrect', (oldVal) => oldVal + 1);
              }
              store.session.transact('trialNumTotal', (oldVal) => oldVal + 1);
...
export const practiceFeedbackIncorrectAFC = (corpusName, assessment_stage_val) => {
  let stim = {
    type: jsPsychAudioMultiResponse,
    response_allowed_while_playing: true,
    data: {
    ...
    stimulus: () => {
      return mediaAssets.audio[dashToCamelCase('afc-' + store.session.get('nextStimulus').audio + '-incorrect')];
    },
    prompt: () => {
    ...
    on_load: () => {
      ...
      buttons.forEach((button) => {
        if (button.textContent.trim() === store.session.get('nextStimulus').target) {
