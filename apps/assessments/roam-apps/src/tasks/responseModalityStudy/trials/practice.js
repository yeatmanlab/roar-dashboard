...
export const practice = (subCorpusName, idx) => {
  return {
    ...
    prompt: () => {
      let timeText = ``;
      if (idx === 0) {
        let continueText = isMobile ? 'responseModalityStudy.practice.text10' : 'responseModalityStudy.practice.text9';
      ...
      if (subCorpusName === 'rtControl_production') {
        ...
        <div class="jspsych-content-modified instructions-bg">
            <h2 class="title">${i18next.t('responseModalityStudy.practice.text1')}</h2>
            <div class="row">
              <div class="instruction-boxes fade-in-1" style="flex-basis: 80%;" id="panel3">
                  <img src="${mediaAssets.images.keyboardExample5}" alt="arrow keys">
        ...
        let mode = isMobile ? 'responseModalityStudy.practice.text4' : 'responseModalityStudy.practice.text3';
        ...
        if (store.session.get('rtControlMouseInstruction')) {
          let mode2 = isMobile ? 'responseModalityStudy.practice.text16' : 'responseModalityStudy.practice.text15';
          afcInstruction = `<p class="instructions-text">${i18next.t('responseModalityStudy.practice.text12', {
            type: `${i18next.t(type)}`,
            mode: `${i18next.t(mode2)}`,
          })}</p>`;
        } else {
          store.session.set('rtControlMouseInstruction', true);
          afcInstruction = `<p class="instructions-text"> ${i18next.t('responseModalityStudy.practice.text2', {
            mode: `${i18next.t(mode)}`,
          })}</p>`;
        }
        return (
