 ...
 *
 * Keeps only non-null, non-empty values (so `0` and `false` are preserved but `null`,
 * `undefined`, and `''` are dropped), and returns `undefined` when nothing was provided so
 * `startRun` is called with no metadata rather than an empty object. This context lives on the
 * run only — it is never written to the user record.
 *
