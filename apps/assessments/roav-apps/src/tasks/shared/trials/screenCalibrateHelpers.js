...
import '../../../i18n/i18n';
import { fillTextKeyValuesDef, ModeGame, NameTask, TAG_REQ_DEF, TypeKey, TypeSize } from '../helpers/namingHelpers';
  ...
  const prepareParams = () => {
    ...
    params = {
      ...fillTextKeyValuesDef(
        paramsTrialScreenMeasureWidth(paramsIn.tagReq, paramsIn.tagModeGame, paramsIn.tagNameTask),
      ),
        ...
        timeline: [
          {
            ...
            on_load: () => {
              const selWidthScreen = document.getElementById('id-width-screen-cm');
              const widthStep = 0.5;
              for (let width = SCREEN.WIDTH_CM_MIN; width <= SCREEN.WIDTH_CM_MAX; width += widthStep) {
