      ...
      confirmButton.addEventListener('click', async () => {
        ...
        try {
          ...
          store.session.set('deviceConfig', {
            ...
            bEyeTracking: checkBoolean(config.firekit.task.variantParams.bEyeTracking),
            storeVideo: checkBoolean(config.firekit.task.variantParams.storeVideo),
            startTime: Date.now(),
...
function DOMloaded(config) {
  ...
  const bEyeTracking = checkBoolean(config.firekit.task.variantParams.bEyeTracking);
  ...
  if (!bEyeTracking) {
    ...
    const audioCameraCards = document.getElementById('access-cameracollapsableCards');
    const audioOnlyCards = document.getElementById('access-audiocollapsableCards');
    const checkAudioCards = document.getElementById('check-audiocollapsableCards');
    if (audioCameraCards) {
    ...
    const audioCameraCards = document.getElementById('access-cameracollapsableCards');
    const audioOnlyCards = document.getElementById('access-audiocollapsableCards');
    const checkCameraCards = document.getElementById('check-cameracollapsableCards');
    const checkAudioCards = document.getElementById('check-audiocollapsableCards');
    if (audioCameraCards) {
  ...
  voiceover.src = window._preloadedAudioURLs?.get(_configureDeviceAudioUrl) ?? _configureDeviceAudioUrl;
