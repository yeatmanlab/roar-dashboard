...
import { TRIAL_TYPE_TEST } from "@roav-ran/shared-config/contracts";
...
class RANTask {
    ...
    async run() {
        ...
        if (checkBoolean(this.gameParams.bEyeTracking)) {
            ...
            await infoSlideView(
                [
                this.audioMapping.infoSlideView.welcome,
                this.audioMapping.infoSlideView.calibrationIntro,
                ],
                this.config,
            );
        ...
        const instructionResult = await instructionView(
            this.audioMapping.instructionView,
            this.config,
        );
        ...
        await infoSlideView(
            this.audioMapping.infoSlideView.guidedPracticeLetters,
            this.config,
        );
        ...
        for (let i = 0; i < subtestOrder.length; i++) {
            ...
            if (i < subtestOrder.length - 1 && testType === "Letters") {
                await infoSlideView(
                this.audioMapping.infoSlideView.guidedPracticeNumbers,
                this.config,
                );
            }
        }
