...
const props = defineProps({
  taskId: { type: String, default: LETTER_TASK_IDS.EN },
  language: { type: String, default: 'en' },
  task: { type: String, default: 'letter' },
...
async function startTask(selectedAdmin) {
  try {
    ...
    // in the new backend. The fallback (matching by letter task UUID within embedded tasks)
    ...
    if (taskRes.status !== 200) {
      throw new Error(`letter task not found in the ROAR backend (status ${taskRes.status}).`);
    }
    if (meRes.status !== 200) {
