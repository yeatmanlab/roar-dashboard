...
import { version } from '@roar-platform/roam-apps/package.json';
...
onMounted(async () => {
  try {
    TaskLauncher = (await import('@roar-platform/roam-apps')).TaskLauncher;
  } catch (error) {
...
async function startTask(selectedAdmin) {
  try {
    ...
    if (taskRes.status !== 200) {
      throw new Error(`ROAM task "${props.taskId}" not found in the ROAR backend (status ${taskRes.status}).`);
    }
    if (meRes.status !== 200) {
    ...
    if (props.launchId) {
      throw new Error(
        'Proxy-launch path is not yet supported for ROAM tasks. Resolve the participant Postgres UUID before enabling this path.',
      );
    }
