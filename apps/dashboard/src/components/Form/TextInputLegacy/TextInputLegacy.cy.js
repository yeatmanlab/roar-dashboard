import TextInputLegacy from './TextInputLegacy.vue';

describe('<TextInputLegacy />', () => {
  it('Renders the input with default attributes', () => {
    cy.mount(TextInputLegacy);
  ...
  it('Renders the input with pre-defined model value', () => {
    cy.mount(TextInputLegacy, { props: { modelValue: 'mock-default-value' } });
  ...
  it('Applies the invalid class when isInvalid is true', () => {
    cy.mount(TextInputLegacy, { props: { isInvalid: true } });
  ...
  it('Applies the invalid class when errors exist', () => {
    cy.mount(TextInputLegacy, { props: { errors: [{ $message: 'Error message' }] } });
    cy.findByTestId('textinput__input-wrapper').get('input').should('have.class', 'p-invalid');
    cy.findByTestId('textinput__input-wrapper').get('input').should('have.class', 'border-red-500');
  });

  it('Renders error messages', () => {
    cy.mount(TextInputLegacy, { props: { errors: [{ $message: 'Error 1' }, { $message: 'Error 2' }] } });
  ...
  it('Disables the input when disabled is true', () => {
    cy.mount(TextInputLegacy, { props: { disabled: true } });
