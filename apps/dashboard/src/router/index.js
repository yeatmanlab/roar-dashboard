...
router.beforeEach(async (to, from, next) => {
  ...
  //
  // Super admins bypass route-permission checks. Read the flag from the `/me`
  // payload already awaited above (the canonical backend source) rather than only
  // from `userCan` → `authStore.userClaims`, which is populated by a *separate*
  // async `resolveUserClaims('/me')` in App.vue that settles after the first
  // navigation. That timing gap is what bounced the first navigation (and made it
  // work only on the second click) on the emulator stack, where super-admin is the
  // sole route-access signal (the emulator token carries no role claim).
