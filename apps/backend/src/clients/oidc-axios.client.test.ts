...
describe('createOidcAxiosInstance', () => {
  ...
  it('throws when Authorization token is missing in production', async () => {
    ...
    await expect(interceptorFn(mockConfig)).rejects.toThrow(
      'OIDC identity token missing, cannot send unauthenticated request',
    );

    process.env.NODE_ENV = originalNodeEnv;
  });

  it('propagates errors from getRequestHeaders', async () => {
    mockGetRequestHeaders.mockRejectedValueOnce(new Error('Auth service unavailable'));
    ...
    await expect(interceptorFn(mockConfig)).rejects.toThrow('Auth service unavailable');
  });
