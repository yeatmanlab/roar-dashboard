...
import type { Administration } from '../../db/schema/core/administrations';
...
export function ReportService({
   ...
   * administration returns 404, not a misleading 403.
   *
   * Returns the loaded administration so callers that already need it (e.g.,
   * `getIndividualStudentReport`) can reuse the lookup instead of issuing a
   * second `getById` round-trip. Callers that don't need it can simply
   * `await` the call and discard the return value.
   *
   * @param authContext - User's auth context
   * @param administrationId - The administration ID to verify
   * @returns The loaded administration record
   * @throws {ApiError} NOT_FOUND if administration doesn't exist
   * @throws {ApiError} FORBIDDEN if user lacks can_read_scores on the administration
   */
  async function verifyAdministrationScoreReadAccess(
