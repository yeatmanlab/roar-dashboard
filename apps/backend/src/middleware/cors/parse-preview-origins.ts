...
/**
 * Converts a preview-origin entry into an anchored matcher, or returns `null`
 * when the entry does not satisfy the grammar.
 *
 * Firebase preview channel URLs take the form
 * `https://{site}--{channel}-{hash}.web.app`.
 *
 * Three properties of the produced pattern are load-bearing:
 *
 ...
 * - **It is anchored.** `^`/`$` are what stop `…web.app.attacker.com` from
 ...
 *
 * Note the `*` never reaches the compiled pattern: it is a marker in config that
 * this function replaces with an explicit character class, so no part of the
 * configured value is treated as a regex.
 */
export function toPreviewOriginPattern(entry: string): RegExp | null {
