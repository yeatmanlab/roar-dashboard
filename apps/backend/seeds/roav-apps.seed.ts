 ...
 * keys match the gameParams accepted by roav-apps' serve.js. The set of tasks to create
 ...
 *
 * Usage:
 *   TASK_VARIANT_PARAMETERS_FILE=./taskVariantParameters.json npm run db:seed:roav-apps -w apps/backend
 *
 * To get started, copy taskVariantParameters.example.json in the assessment directory:
 *   cp apps/assessments/roav-apps/taskVariantParameters.example.json \
 *      apps/assessments/roav-apps/taskVariantParameters.json
 *
...
async function seedTask(taskId: string): Promise<{ id: string }> {
  const meta = TASK_META[taskId as RoavAppsTaskId];
