...
// Deterministic IDs live in fixture-ids.ts. Imported for use below and re-exported
// so existing importers of `DEV_IDS` from './fixture' keep working.
import { DEV_IDS } from './fixture-ids';

export { DEV_IDS };

/**
 * Real, public legal documents for the fixture's consent and assent agreements.
 *
 * `agreement.service.ts` resolves agreement content by fetching
 * `raw.githubusercontent.com/<orgRepo>/<sha>/<filename>` and throws on a non-200.
 * Placeholder coordinates therefore 500 the content endpoint, and because the
 * participant home blocks its task list until the consent gate resolves, a fixture
 * student sees no tasks at all. Pinned to a commit whose files are verified to
 * resolve; the documents are public, so this needs network but no credentials.
 */
const LEGAL_DOCS_ORG_REPO = 'yeatmanlab/roar-legal-documents';
const LEGAL_DOCS_COMMIT_SHA = 'c9013f55d0f2c6b6c40ac8824546e58c94bb2682';
...
export async function seedDevFixture(): Promise<BaseFixture> {
  ...
  const [
    ...
    // Dedicated to exercising the dashboard launch path. Seeded empty — `dev:seed:tasks`
    // assigns each assessment's `defaultVariant` here, so it holds only real, launchable
    // tasks. The other administrations keep their synthetic `TaskFactory` tasks, whose
    // generated slugs (`task-3-a3f9k2`) drive progress and score fixtures but resolve to
    // no launch route. Named to sort first so a participant lands on it by default.
    AdministrationFactory.create({
      id: DEV_IDS.administrationLaunch,
      name: 'Assessment Launch Sandbox (Dev)',
      namePublic: 'Assessment Launch Sandbox',
      createdBy: districtAdmin.id,
    }),
  ]);
  ...
  await Promise.all([
    ...
    // District-level so every student under District A — School A and Class A alike —
    // sees the launchable assessments.
    AdministrationOrgFactory.create({ administrationId: administrationForLaunch.id, orgId: district.id }),
  ...
  await Promise.all([
    AgreementVersionFactory.create(
      {
        id: DEV_IDS.consentVersion,
        locale: 'en-US',
        githubOrgRepo: LEGAL_DOCS_ORG_REPO,
        githubCommitSha: LEGAL_DOCS_COMMIT_SHA,
        githubFilename: 'studentConsent.md',
      },
      { transient: { agreementId: consentAgreement.id } },
    ),
    AgreementVersionFactory.create(
      {
        id: DEV_IDS.assentVersion,
        locale: 'en-US',
        githubOrgRepo: LEGAL_DOCS_ORG_REPO,
        githubCommitSha: LEGAL_DOCS_COMMIT_SHA,
        githubFilename: 'studentAssent.md',
      },
