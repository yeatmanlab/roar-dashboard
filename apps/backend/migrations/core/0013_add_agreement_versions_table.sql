CREATE TABLE "app"."agreement_versions" (
	...
	CONSTRAINT "agreement_versions_locale_format" CHECK ("app"."agreement_versions"."locale" ~ '^[a-z]{2}$')
);
--> statement-breakpoint
ALTER TABLE "app"."agreement_versions" ADD CONSTRAINT "agreement_versions_agreement_id_agreements_id_fk" FOREIGN KEY ("agreement_id") REFERENCES "app"."agreements"("id") ON DELETE restrict ON UPDATE no action;--> statement-breakpoint
CREATE UNIQUE INDEX "agreement_versions_current_unique_idx" ON "app"."agreement_versions" USING btree ("agreement_id","locale") WHERE "app"."agreement_versions"."is_current" = true;--> statement-breakpoint
CREATE UNIQUE INDEX "agreement_versions_identity_unique_idx" ON "app"."agreement_versions" USING btree ("agreement_id","locale","github_commit_sha");--> statement-breakpoint
