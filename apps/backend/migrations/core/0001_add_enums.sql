...
CREATE TYPE "app"."auth_provider" AS ENUM('password', 'google', 'oidc.clever', 'oidc.classlink', 'oidc.nycps');--> statement-breakpoint
...
CREATE TYPE "app"."rostering_provider" AS ENUM('classlink', 'clever', 'dashboard', 'nycps', 'csv');--> statement-breakpoint
CREATE TYPE "app"."school_level" AS ENUM('early_childhood', 'elementary', 'middle', 'high', 'postsecondary');--> statement-breakpoint
CREATE TYPE "app"."score_type" AS ENUM('computed', 'raw');--> statement-breakpoint
CREATE TYPE "app"."task_variant_status" AS ENUM('draft', 'published', 'deprecated');--> statement-breakpoint
CREATE TYPE "app"."trial_interaction_type" AS ENUM('focus', 'blur', 'fullscreen_enter', 'fullscreen_exit');--> statement-breakpoint
CREATE TYPE "app"."user_family_role" AS ENUM('parent', 'child');--> statement-breakpoint
CREATE TYPE "app"."user_role" AS ENUM('site_administrator', 'administrator', 'teacher', 'student');--> statement-breakpoint
