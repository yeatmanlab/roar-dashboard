...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_foreign_server WHERE srvname = 'assessment_server') THEN
    CREATE SERVER assessment_server
      FOREIGN DATA WRAPPER postgres_fdw
      OPTIONS (host '$PG_HOST', port '$PG_PORT', dbname '$ASSESSMENT_DB');
  ELSE
    ALTER SERVER assessment_server
      OPTIONS (SET host '$PG_HOST', SET port '$PG_PORT', SET dbname '$ASSESSMENT_DB');
  END IF;
END
\$\$;
