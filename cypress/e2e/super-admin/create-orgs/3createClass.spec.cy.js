...
function inputParentOrgDetails() {
  ...
  cy.get('ul > li', { timeout: Cypress.env('timeout') })
    .contains(Cypress.env('testDistrictName'))
    .click();
  cy.get('[data-cy="dropdown-parent-school"]', { timeout: Cypress.env('timeout') })
    .wait(1000)
    .click();
  cy.get('ul > li', { timeout: Cypress.env('timeout') })
    .contains(Cypress.env('testSchoolName'))
    .click();
}
...
describe('The admin user can navigate to the create organizations page, and create a new class.', () => {
  it(
    'Navigates to the create organizations page, creates a new class, and checks that the created ' +
    ...
    () => {
      ...
      cy.inputOrgDetails(
        Cypress.env('testClassName'),
        Cypress.env('testClassInitials'),
        null,
        null,
        Cypress.env('testGrade'),
        Cypress.env('testTag'),
      );
