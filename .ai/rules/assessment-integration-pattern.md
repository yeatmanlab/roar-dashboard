...
| Hosting   | `.firebaserc`                                                           | Hosting target under **both** `gse-roar-admin` and `gse-roar-admin-staging` — two edits in one file                       |
| Hosting   | `apps/assessments/<name>/firebase.json`                                 | `"target": "<name>"` in the `hosting` block, matching the `.firebaserc` target                                            |
...
`.firebaserc` is the same trap in a different file: it lists every assessment **twice**, once under each project. The target name is the assessment's directory name (and so the CI matrix value), and the site ID it maps to differs per project — that's what lets one target resolve to the staging or the production site. Add the staging block only and staging deploys fine while production fails at deploy time with `Hosting site or target <name> not detected in firebase.json`.
