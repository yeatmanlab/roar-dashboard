...
const loadHyperparameters = async (task) => {
  return new Promise((resolve, reject) => {
    const filename =
      task === "cva"
        ? "roar_cva_hyperparameters"
        : "roar_morphology_hyperparameters";
...
export const initializeClowder = async () => {
  ...
  if (isAdaptive) {
    try {
      ...
      console.warn(
        `Failed to load hyperparameters: ${err.message}. Proceeding with unscaled thetas.`,
      );
    }
  }
  ...
  if (isAdaptive && task === "morphology") {
    ...
    catsConfig = {
      ...
      composite_comprehension: {
        ...
        priorDist: "norm",
        startSelect: startItemSelect,
        ...(hyperParams.composite_comprehension
          ? hyperParams.composite_comprehension
          : {}),
    ...
    catsConfig = {
      ...
      composite_comprehension: {
        ...
        randomSeed: randomSeed
          ? `${randomSeed}-composite-comprehension`
          : randomSeed,
      },
    };
  } else {
...
export const scaleTheta = (
  ...
  if (!taskHyperParams) {
    return [
      thetaRaw,
      clampPositive(thetaSERaw),
      thetaComprehensionRaw,
      clampPositive(thetaSEComprehensionRaw),
    ];
  }
  ...
  if (comprehensionHyperParams) {
    const {
      transformationScale: comprehensionScale,
      transformationShift: comprehensionShift,
    } = comprehensionHyperParams;
    thetaScaledComprehension =
      thetaComprehensionRaw * comprehensionScale + comprehensionShift;
    thetaSEScaledComprehension =
      thetaSEComprehensionRaw * Math.abs(comprehensionScale);
  }
...
export const setNextStimulus = () => {
  ...
  const forceSecondaryBehavior =
    store.session.get("config").forceSecondaryBehavior;
