...
const breakScreen = {
    ...
    if (store.session.get("isK2")) {
      return `
       <div class = "jspsych-content-modified">
          <img src="${
            mediaAssets.images["coreMathBreakScreen" + breakCount]
              ...
              <div class="text-image">
                <div>
                  <h1 class="header">${i18next.t(
                    "gameBreak.core-math.break" + breakCount + ".K2-text1",
                  )} </h1>
                  <p class="text"> ${i18next.t(
                    "gameBreak.core-math.break" + breakCount + ".K2-text2",
                  )} </p>
                </div>
      ...
      let breakScreenPath =
        "coreMathBreakScreen" +
        store.session.get("breakScreenNames")[breakCount];
      return `
        <div class = "jspsych-content-modified">
          <img src="${
            mediaAssets.images[breakScreenPath]
          }" alt= "background" class="imageBG"> 
          <div class="tiger-gif-container">
            <div class="speechbubble">
              <h1 class="header">${i18next.t(
                "gameBreak.core-math.break" + breakCount + ".text1",
              )} </h1>
  ...
  on_finish: (data) => {
    jsPsych.data.addDataToLastTrial({
      save_trial: true,
      correct: 1,
      assessment_stage: "break_response",
    });
...
export const ifBreakScreen = {
  ...
  conditional_function: () => {
    if (
      ...
      store.session.get("indexTracking") ===
        store.session.get("breakMap")[store.session.get("breakCount")]
    ) {
