...
const HomeParticipant = defineAsyncComponent(() =>
  import("@/pages/HomeParticipant.vue"),
);
const HomeAdministrator = defineAsyncComponent(() =>
  import("@/pages/HomeAdministrator.vue"),
);
const ConsentModal = defineAsyncComponent(() =>
  import("@/components/ConsentModal.vue"),
);
...
const consentType = computed(() => {
  if (isAdminUser.value) {
    ...
    return i18n.locale.value.includes("es")
      ? CONSENT_TYPES.ASSENT_ES
      : CONSENT_TYPES.ASSENT;
  }
});
...
async function checkConsent() {
  ...
  const signedBeforeAugFirst = legalDocs.some((doc) =>
    isSignedBeforeAugustFirst(doc.dateSigned),
  );
