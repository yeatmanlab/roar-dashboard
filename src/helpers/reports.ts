...
export const extendedDescriptions = {
  ...
  vocab:
    "This test measures how well your student knows words by having them match a picture to a spoken word.",
  ...
  syntax:
    "This test measures how well students understand sentences that vary from simple to complicated",
...
export const addElementToPdf = async (
  ...
  await html2canvas(element, { windowWidth: 1300, scale: 2 }).then(
    function (canvas) {
      const imgData = canvas.toDataURL("image/jpeg", 0.7, {
        willReadFrequently: true,
      });
      const scaledCanvasHeight =
        canvas.height * returnScaleFactor(canvas.width);
...
export const taskInfoById = {
  ...
  pa: {
    ...
    desc:
      ...
      `${getRawScoreRange("pa").min}-${
        getRawScoreRange("pa").max
      } and can be ` +
  ...
  sre: {
    ...
    desc:
      ...
      ` The student's score will range between ${getRawScoreRange("sre").min}-${
        getRawScoreRange("sre").max
      } ` +
