                        ...
                        },
                        {
                            fieldFilter: {
                                ...
                                value: {
                                    arrayValue: {
                                        values: [
                                            parentSchools.map((schoolId) => {
                                                return {
                                                    stringValue: schoolId,
                                                };
                                            }),
                                        ],
                                    },
...
export const orgCounter = async (
    ...
    if (isSuperAdmin.value) {
        ...
        return axiosInstance
            ...
            .then(({ data }) => {
                return Number(
                    convertValues(data[0].result?.aggregateFields?.count)
                );
            });
    } else {
        ...
        if (activeOrgType.value === "districts") {
            ...
            if (selectedSchool.value) {
                return fetchDocById("schools", selectedSchool.value, [
                    "classes",
                ]).then((school) => {
...
export const orgFetcher = async (
    ...
    if (isSuperAdmin.value) {
        ...
        const requestBody = getOrgsRequestBody({
            orgType: orgType,
            parentDistrict:
                orgType === "schools" ? selectedDistrict.value : null,
            aggregationQuery: false,
            paginate: false,
            select: select,
        });
        ...
        if (["groups", "families"].includes(orgType)) {
            ...
            for (const districtId of districtIds) {
                promises.push(
                    fetchDocById(orgType, districtId, ["name", "id"])
                );
            }

            return Promise.all(promises);
        } else if (orgType === "schools") {
            const districtDoc = await fetchDocById(
                "districts",
                selectedDistrict.value,
                ["schools"]
            );
            if (
                (adminOrgs.value["districts"] ?? []).includes(
                    selectedDistrict.value
                )
            ) {
...
export const orgPageFetcher = async (
    ...
    if (isSuperAdmin.value) {
        ...
        if (
            ...
            ((adminOrgs.value["schools"] ?? []).includes(
        ...
        const orderField = (orderBy?.value ?? orderByDefault)[0].field
            .fieldPath;
