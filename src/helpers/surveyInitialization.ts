...
export async function setupStudentAudio(
  ...
  await fetchBuffer({ 
    ...
    setSurveyAudioPlayerBuffers: surveyStore.setSurveyAudioPlayerBuffers as (locale: string, buffers: AudioBuffer[]) => void, 
  });
  ...
  surveyInstance.onAfterRenderPage.add((sender: SurveyModel, options: { htmlElement: HTMLElement }) => {
    ...
    questionElements.forEach((el) => {
      ...
      showAndPlaceAudioButton({playAudioButton: playAudioButton as HTMLElement | null, el: htmlEl});
    });
  });
}
...
export function setupSurveyEventHandlers({
  ...
  surveyInstance.onCurrentPageChanged.add((sender: SurveyModel, options: { oldCurrentPage: PageModel | null, newCurrentPage: PageModel, isNextPage: boolean, isPrevPage: boolean }) => {
