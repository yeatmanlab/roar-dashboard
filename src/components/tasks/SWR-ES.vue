...
async function startTask() {
  ...
  const appKit = await authStore.roarfirekit.startAssessment(currentAssignment.value, "swr-es")
  ...
  await roarApp.run().then(async () => {
    ...
    await authStore.roarfirekit.completeAssessment(currentAssignment.value, "swr-es")
    router.replace({ name: "Home" });
  });
}
</script>
