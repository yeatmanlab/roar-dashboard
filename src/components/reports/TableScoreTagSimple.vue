<template>
  ...
  >
    <PvTag
      ...
      :severity="_get(colData, severityField)"
      :value="_get(colData, col.field)"
      :icon="_get(colData, iconField)"
      :style="`background-color: ${_get(colData, tagColor)}; min-width: 2rem; ${
        returnScoreTooltip(col.header, colData, col.field).length > 0 &&
        'outline: 1px dotted #0000CD; outline-offset: 3px'
      }`"
      rounded
    />
    <div v-else-if="col.tagOutlined && _get(colData, tagColor)" class="circle" style="border: 1px solid black" />
  </div>
  <div v-if="col.emptyTag" v-tooltip.right="`${returnScoreTooltip(col.header, colData, col.field)}`">
    <div
      v-if="!col.tagOutlined"
      class="circle"
      :style="`background-color: ${_get(colData, tagColor)}; color: ${
    ...
    <div
      v-else-if="col.tagOutlined && _get(colData, tagColor)"
      class="circle"
...
console.log('tablescoretag', props.colData, props.col);
let returnScoreTooltip = (colHeader, colData, fieldPath) => {
  ...
  if (selectedTaskId && colData[selectedTaskId]?.support_level) {
    // Handle scored tasks
