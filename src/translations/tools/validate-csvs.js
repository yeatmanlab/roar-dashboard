...
function main() {
  ...
  for (const file of files) {
    ...
    for (const row of rows) {
      ...
      if (globalIds.has(id))
        throw new Error(`${file}: duplicate identifier across files: ${id} (also in ${globalIds.get(id)})`);
  ...
  console.log(
    `Validated ${files.length} CSV files, ${totalRows} identifiers, locales: ${Array.from(allLocales).join(', ')}`,
  );
  // Coverage report
