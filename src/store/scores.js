  ...
  switch(run.taskId) {
    ...
    case "sre":
      ...
      return { 
        roarScore: thetaToRoarScore(run.thetaEstimate),
        normedPercentile: normedPercentile,
        supportLevel: percentileToSupportClassification("sre", normedPercentile, run.grade),
      };
    case "vocab":
    default:
...
export function percentileToSupportClassification(taskId, percentile, grade=1) {
  ...
  switch(taskId) {
    ...
    case "sre":
        support = (percentile <= 2 ) ? "Very Low" : (percentile <= 8) ? "Low": (percentile <= 23) ? "Below Average": (percentile <= 75) ? "Average": "Above Average";
      ...
      sreStats: (state) => { 
        ...
        return { 
          ...
          tosrecSupport: {
            ...
            belowAverage: tosrecSupport.filter(x => x === "Below Average").length,
            low: tosrecSupport.filter(x => x === "Low").length,
            veryLow: tosrecSupport.filter(x => x === "Very Low").length
          },
          ...state.ageStats,
          ...state.gradeStats,
          ...state.roarScoreStats,
