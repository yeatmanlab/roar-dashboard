...
describe('toPaScoreEntries', () => {
    ...
    it('emits roarScoreKind and scoringVersion when present in subtask scores', () => {
      const computed = {
        fsm: {
          ...
          numAttempted: 15,
          percentCorrect: 67,
          roarScoreKind: 'raw_total_correct',
          scoringVersion: 4,
        },
      };
  ...
  describe('composite scores', () => {
    ...
    it('composite and composite_foundational land under distinct domains', () => {
      ...
      const roarScoreEntries = entries.filter((e: PaScoreEntry) => e.name === PA_SCORE_NAMES.RAW_SCORE);
    ...
    it('skips null/undefined composite scores', () => {
      ...
      expect(entries).not.toContainEqual(expect.objectContaining({ name: PA_SCORE_NAMES.PERCENTILE }));
      expect(entries).not.toContainEqual(expect.objectContaining({ name: PA_SCORE_NAMES.PERCENTILE_SPR }));
    });
  });
  ...
  describe('strict mode validation', () => {
    ...
    it('throws on unregistered input group key in strict mode', () => {
      ...
      expect(() => toPaScoreEntries(computed, { strict: true })).toThrow(/unregistered_group/);
    });
  });
  ...
  describe('edge cases', () => {
    ...
    it('converts numeric values to strings', () => {
      ...
      expect(entries).toContainEqual(expect.objectContaining({ name: PA_SCORE_NAMES.NUM_CORRECT, value: '10' }));
      expect(entries).toContainEqual(expect.objectContaining({ name: PA_SCORE_NAMES.PERCENT_CORRECT, value: '67.5' }));
    });
